...
export function AgentCard({ skill, featured, savedInLibrary }: { skill: Skill; featured?: boolean; savedInLibrary?: boolean }) {
  const t = useT();
      ...
      <a href={`/agents/${skill.slug}`} className="block p-4 sm:p-5 flex-1">
        ...
        <div className="flex items-start justify-between mb-3">
          ...
          <div className="flex items-center gap-1.5 shrink-0 ml-2">
            <span className="text-xs px-2 py-0.5 rounded-full border bg-orange-500/10 border-orange-500/20 text-orange-400">
              agent
            </span>
            {savedInLibrary !== undefined && (
              <SaveButton type="agent" itemSlug={skill.slug} initialSaved={savedInLibrary} size="sm" />
            )}
          </div>
        </div>
